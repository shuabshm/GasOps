...
# 3. **Key Insights** (CONDITIONAL - skip on follow-up and also do not mention any headings like "key insights" or "takeaways" or similar):
...
The response structure is determined by the user's explicit intent. **Do not mention any headings like "key insights" or "takeaways" or similar.**
...
1.  **One-Sentence Answer (Action-Oriented):** Provide a concise, direct, one-sentence summary answer to the user's question.
    * **If pending > 0 (action needed):** "[X] welds are pending updates in work order [Y] ([Z] already completed)"
    * **If all completed (no action needed):** "All [X] unlocked welds in work order [Y] have been updated"
    * *Highlight what needs action first!* Use total record count for totals.
...
1.  **One-Sentence Answer:** Provide a concise, direct, one-sentence confirmation specific to the data being displayed.
    * Example: “Here is the complete list of pending and completed unlocked welds.”
