...
from prompts.weld_apis_prompts.GetWeldMapStatus import get_api_prompt as get_weld_map_prompt
...
def get_data_analysis_prompt(user_input, analysis_results, api_name=None, is_follow_up=False):
    ...
    # **ADD THIS: Special handling for weld map queries**
    # Check if this is a weld map query by looking for weld map-specific data
    is_weld_map_query = analysis_results.get("is_weld_map_query", False)
    
    if is_weld_map_query:
        logger.info("Detected weld map query - using weld map prompt")
        # Add weld map statistics to data representation
        data_representation["total_unique_welds"] = analysis_results.get("total_unique_welds", 0)
        data_representation["added_to_map_yes"] = analysis_results.get("added_to_map_yes", 0)
        data_representation["added_to_map_no"] = analysis_results.get("added_to_map_no", 0)
        data_representation["weld_map_records"] = analysis_results.get("weld_map_records", [])
    ...
    # **ADD THIS: Check for weld map first, before other API checks**
    if is_weld_map_query:
        api_specific_prompt = get_weld_map_prompt()
