def get_api_prompt(api_parameters=None):
    """
    Returns the API-specific prompt for GetWorkOrderRejactableTRIndicationsbyCriteria API
    ...
    return f"""
=== GetWorkOrderRejactableTRIndicationsbyCriteria API - SPECIFIC GUIDELINES ===
**IMPORTANT: Use ONLY these guidelines below for this API. Ignore any other API instructions section.**

This API returns **rejectable** TR (Tertiary Review) indication details with flexible grouping, showing counts of critical quality defects that require attention.
...
- Indication: Type of rejectable TR indication (e.g., Other (enter in remarks), Porosity, Slag Inclusions, etc.)
- TRName: TR inspector name (can be filter or GroupBy field)
...
- TRName: Hide if used as filter UNLESS it's in GroupBy
...
| ["TRName", "Indication"] | Inspector detection patterns for rejectable defects, rejection consistency |
...
1.  **One-Sentence Answer:** Provide a concise, direct, one-sentence summary answer to the user's specific question from a business perspective.
    ...
    * Examples:
        * "Work order 100500514 has 2 rejectable TR indication types, with Other (enter in remarks) being the most critical at 1 occurrence."
...
1.  **One-Sentence Answer:** Provide a concise, direct, one-sentence confirmation specific to the data being displayed.
    * Example: "Here is the full table of rejectable TR indications, sorted by criticality/frequency."
...
=== END GetWorkOrderRejactableTRIndicationsbyCriteria GUIDELINES ===
"""
