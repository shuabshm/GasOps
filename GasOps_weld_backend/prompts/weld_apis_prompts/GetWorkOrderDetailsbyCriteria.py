...
def get_api_prompt(api_parameters=None):
    ...
    """
    filter_info = api_parameters if api_parameters else {}
    hide_project = "ProjectNumber" in filter_info

    return f"""
=== GetWorkOrderDetailsbyCriteria API ===

This API returns work order details filtered by Heat Serial Number, NDE Report Number, Weld Serial Number, or Project Number.

FIELDS TO DISPLAY:
- Always show WorkOrderNumber and Location
- Show ProjectNumber only if it is NOT used as a filter
- Hide ProjectNumber if filtered (all rows will have same value)

RESPONSE FORMAT:
- Provide a one-line human-friendly summary of total work orders for the filter criteria.
  Example:
    - With ProjectNumber filter: "For project G-23-901, 4 work orders found. Below are the details."
    - Other filters: "Found 5 work orders for heat number 648801026."
- Display a table of all rows with the following columns:
    - { 'ProjectNumber' if not hide_project else '' }
    - WorkOrderNumber
    - Location
    
- Flatten all values; use "-" for missing/empty fields
- No extra commentary, insights, or follow-up questions
- Show all rows; sort as received with first column priority

CRITICAL RULES:
- Use words like "work orders" or "records" — never "dataset"
- Only display the requested columns as per above logic
- Single, concise response with table
"""
